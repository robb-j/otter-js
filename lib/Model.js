...
class Model {
  ...
  async save() {
    ...
    // Let the model validate itself
    if (this.isValid() === false) {
      throw new Error(`${this.modelName}: Cannot save because validation failed`)
    }
  ...
  async destroy() {
    ...
    if (undefOrNull(this.id)) {
      ...
      if (this.canBeDestroyed() !== true) {
        throw new Error(`${this.modelName}: Cannot destroy model '${this.id}' because canBeDestroyed returned false`)
      }
  ...
  /** Whether the model can be destroyed or not */
  canBeDestroyed() { return true }
  
  /* istanbul ignore next */
  /** Whether the model can be saved or not */
  isValid() { return true }
}
