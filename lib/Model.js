...
class Model {
  ...
  /** Experimenting with custom promises to add chainable configuration */
  static find2(rawQuery = {}) {
    let query = this.adapter.makeQuery(this.name, rawQuery)
    return new QueryPromise((resolve, reject) => {
      process.nextTick(() => {
        this.find(query).then(resolve).catch(reject)
      })
    }, query)
  }
