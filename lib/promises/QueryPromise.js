...
OtterError.registerTypes('queryProm', {
  invalidWhere: (args) => `Invalid Arguements, ${formatObj(args)}`
})
...
class QueryPromise extends Promise {
  ...
  where(...args) {
    ...
    if (args.length === 2) {
      ...
      throw OtterError.fromCode('queryProm.invalidWhere', args)
    }
    return this
  }
  
  first() {
    this.query.singular = true
    this.query.limit = 1
    return this
  }
