...
let Otter = {
  ...
  addModel(...modelTypes) {
    ...
    // Add each model type
    Object.values(modelTypes).forEach(ModelType => {
   ...
   * @return {Otter}
   */
  addCluster(...clusterTypes) {
    
    // Add each cluster type
    Object.values(clusterTypes).forEach(ClusterType => {
   ...
   * @return {Otter}
   */
  addAttribute(...attrTypes) {
    
    // Add each attribute type
    Object.values(attrTypes).forEach(AttrType => {
  ...
  addQueryExpr(exprType, expr) {
    ...
    let newExpr = {
      name: exprType,
      validator: expr,
      precedence: typeof expr.precedence === 'number'
        ? expr.precedence
        : 99
    }
    
    
    // Insert the expression, sorted highest precedence first
    let insertIndex = this.active.exprs.length
    for (let i in this.active.exprs) {
      if (newExpr.precedence > this.active.exprs[i].precedence) {
        insertIndex = i
        break
      }
    }
    ...
    this.active.exprs.splice(insertIndex, 0, newExpr)
