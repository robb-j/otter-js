...
class Attribute {
  ...
  validateModelValue(value) {
    ...
    if (undefOrNull(value)) {
      ...
      else throw AttributeError.fromCode('attr.validation.missing', this)
    }
    
    // Check #valueMatchesType
    if (!this.valueMatchesType(value)) {
      throw AttributeError.fromCode('attr.validation.type', this, value)
    }
    
    // Check enum options
    if (this.enumOptions && !this.enumOptions.includes(value)) {
      throw AttributeError.fromCode('attr.validation.enum', this, value)
    }
    
    // Call validator
    if (this.validator && !this.validator(value)) {
      throw AttributeError.fromCode('attr.validation.custom', this, value)
    }
