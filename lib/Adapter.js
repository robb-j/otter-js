...
class Adapter {
  ...
  /**
   * Adds a processor to the adapter, used to processes Otter Queries into database-specific logic
   * @param {String} name The name of the processor
   * @param {Otter.Types.QueryProcessor} processor The processor to add, the type depending on the Adapter
   */
  addProcessor(name, processor, overwrite = true) {
    ...
    // Only set if in overwrite mode or it is not set
    if (overwrite || !this.processors[name]) {
      
      // Store the processor (with this bound to the adapter)
      this.processors[name] = processor.bind(this)
    }
   ...
   * @return {Promise<Object[]>} The raw matched records
   */
   ...
   * @return {Promise<Object[]>} The raw new records
   */
   ...
   * @param  {object} options The options for the query
   * @return {Promise<Object>} The raw updated records
   */
   ...
   * @param  {object} options The options for the query
   * @return {Promise<number>} The number of records destroyed
   */
  async destroy(modelName, rawQuery, options) { }
  
  /**
   * Counts the number of records that match a query
   * @param  {String}  modelName The name of the model to count from
   * @param  {any|Otter.Types.Query}  rawQuery  The query to count using
   * @param  {object}  options   The options for the query
