...
module.exports = {
  ...
  /** Things that have been activated on the Otter instance,
    these values are shallow copied on extend */
  active: {
    plugins: [],
    models: {},
    adapters: {},
  ...
  use(plugin, ...args) {
    ...
    // If a function was passed, convert to full syntax
    if (typeof plugin === 'function') {
      plugin = { install: plugin }
    }
    
    
    // Fail if the plugin is invalid
    ...
    for (let modelName in this.active.models) {
      ...
      for (let attrName in modelAttributes) {
        ...
        if (!adapter.supportsAttribute(attribute)) {
          let adapterType = utils.className(adapter)
          let attrType = utils.className(attribute)
