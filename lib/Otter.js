...
module.exports = {
  ...
  use(plugin, ...args) {
    ...
    // -> remaining params are the extra ones passed to .use(plugin, ...)
  ...
  extend() {
    ...
    for (let prop in this.active) {
      ...
      // Skip if not one of our own
      if (!this.active.hasOwnProperty(prop)) { continue }
  ...
  spinUp() {
    ...
    // for (let attr in Object.values(attributes)) {
    //   try { this.addAttribute(attr) }
    ...
    //  - Check adapter supports model (check its attributes)
   ...
   * @param {Model} model The model subclass to register
   */
  addModel(model) {
   ...
   * @param {Attribute} attribute The attribute subclass to register
   */
   ...
   * @param {Adapter} adapter The adapter subclass to register
   */
  addAdapter(adapter) {
