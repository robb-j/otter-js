...
const { setOrError, prepareClusters, makePluginable } = utils
...
let Otter = {
  ...
  async start() {
    ...
    // Then prepare models
    prepareClusters(this, this.active.models)
  ...
  async stop() {
    ...
    // Unset schemas on models/models
   ...
   * @param {Otter.Types.Model} model The model class to register
   * @return {Otter}
   */
  addModel(model) {
   ...
   * @param {Otter.Types.Cluster} cluster The cluster class to register
   * @return {Otter}
   */
  addCluster(cluster) {
    
    // Set the cluster or throw an error if it exists
    setOrError(
      cluster,
      cluster.name,
      this.active.clusters,
  ...
  addAttribute(attribute) {
    ...
    setOrError(
      attribute,
      attribute.name.replace(/Attribute$/, ''),
