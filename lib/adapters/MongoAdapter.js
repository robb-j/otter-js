...
class MongoAdapter extends Adapter {
  ...
  genMongoPipeline(query) {
    ...
    if (query.processed) {
      pipeline.push({ $match: this.genMongoMatch(query) })
    }
    
    return pipeline
  }
  
  genQueryAggregates(query) {
    
    let opts = {}
    
    if (typeof query.limit === 'number') {
      opts.$slice = query.limit
    }
    
    if (Array.isArray(typeof query.limit) && query.limit.length === 2) {
      opts.$slice = query.limit
    }
