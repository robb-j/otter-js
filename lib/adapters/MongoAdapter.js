...
const allowedAttributes = [
  'String', 'Number', 'Date', 'Boolean', 'HasOne', 'HasMany', 'ObjectID', 'NestOne', 'NestMany', 'PolyOne', 'PolyMany'
...
class MongoAdapter extends Adapter {
  ...
  * @param {object} schema The schema of the record's Model
  ...
  async count(modelName, rawQuery, options) {
    ...
    // Execute the operation
    let result = await this.db.collection(modelName).count(mongoQuery)
  ...
  genMongoQuery(query) {
    ...
    for (let attrKey in query.processed) {
      ...
      Object.assign(mq, this.evaluateExpr(attrKey, query.processed[attrKey]))
    }
  ...
  evaluateExpr(attrKey, fullExpr) {
    ...
    return exprProc(attrKey, fullExpr.attr, fullExpr.expr)
  }
