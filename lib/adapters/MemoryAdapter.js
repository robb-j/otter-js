...
class MemoryAdapter extends Adapter {
  ...
  async create(modelName, allValues) {
    ...
    // TODO: (untested) Check the type exists
  ...
  makeQuery(modelName, rawQuery, options) {
    let q = super.makeQuery(modelName, rawQuery, options)
    if (typeof q.where.id === 'number') { q.where.id = `${q.where.id}` }
    return q
  }
  ...
  evaluateRecord(record, query) {
    ...
    for (let attrName in query.processed) {
      let expr = query.processed[attrName]
      
      if (!this.evaluateExpr(expr, record[attrName])) {
        return false
      }
    }
    return true
  ...
  evaluateExpr(fullExpr, value) {
    ...
    if (!exprProc) {
      throw new Error(`MemoryAdapter: Unsupported expression ${fullExpr.type}`)
    }
