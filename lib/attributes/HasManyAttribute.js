...
const { undefOrNull, AddTraits, RelationType, setUsingDot } = require('../utils')

/** An Attribute that defines a one-to-many relation to another model, using a HasOne on the other model */
module.exports = class HasManyAttribute extends AddTraits(Attribute, RelationType) {
  ...
  validateSelf(Otter, ModelType) {
    ...
    if (undefOrNull(this.options.model)) {
      throw OtterError.fromCode('attr.relation.missingModel', this)
    }
    ...
    if (!target.attrName) {
      throw OtterError.fromCode('attr.hasMany.noVia', this)
    }
    ...
    if (!targetAttr) {
      throw OtterError.fromCode('attr.hasMany.missingVia', this, target.typeName, target.attrName)
    }
    
    
    // Check it is a HasOne Attribute to
    if (!(targetAttr instanceof HasOne) || targetAttr.options.model !== ModelType.name) {
      throw OtterError.fromCode('attr.hasMany.invalidVia', this, target.typeName, target.attrName)
    }
  }
  ...
  processOptions(Otter, ModelType) {
    ...
    this.options.targetModel = this.relatedType(Otter, target.typeName)
  }
  ...
  getRelation(Otter) {
    ...
    let viaMatch = this.options.model.match(/^(.+) via+ (.+)$/)
  ...
  installOn(model) {
    ...
    relation.remove = function(arg) {
      ...
      if (targetIdAttr.valueMatchesType(arg)) { arg = [ arg ] }
      ...
      if (Array.isArray(arg)) {
        ...
        let isIds = arg.reduce((flag, elem) => flag && targetIdAttr.valueMatchesType(elem), true)
